...
2. **平台骨架稿（命令）**：`content platform-draft --platform xiaohongshu`
   ...
   表达空位；豆包改写版必须原样带禁说清单；
3. **尝菜（发前必做，V1 教训）**：对照骨架稿里的「受众匹配自查」回答——
   ②「谁的任务变了」是不是本账号定位受众（普通年轻技术创作者/学生）？
   不是 → 不发或换技术向出口；核心结论是「你不用管」的内容不做主推送；
   标题里的每个数字必须只有一种解读（DeepSeek「贵一倍」歧义的反面教材）；
4. **配图生成**：按骨架稿的配图清单交给 ai-signal-studio 生成（封面卡片/标注图），
   输出到 Vault 的 `AI Signal/Images/`；
5. **发布**：用户登录平台复制粘贴发布（agent 不代发）；来源链接放评论区；
6. **回流**：发布后用户在选题卡填第二轮反馈，说「同步反馈」，agent 跑
