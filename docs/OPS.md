...
## 动作 8：初稿与发布（DEC-019：小红书先行、并存、复制粘贴）

1. **提示包**（agent 组装，见动作 6 的产物样例）：四问答案 + 必出现事实
   （带一手链接）+ 禁说清单 + 标题候选 + 初稿要求，放进 Vault 的
   `AI Signal/Drafts/`；
2. **平台成品稿**：agent 按平台形状写可直接复制的稿（小红书：标题≤20字 +
   数字清单 + 谁受影响 + 怎么办 + 来源 + 标签 + 配图建议）；豆包改写版必须
   原样带禁说清单；**配图模板（V1 短板）**：官方页/价目表截图 + 红框标注
   关键数字的规格说明，随成品稿一起给；
3. **发布**：用户登录平台复制粘贴发布（agent 不代发）；来源链接放评论区；
4. **回流**：发布后用户在选题卡填第二轮反馈，说「同步反馈」，agent 跑
