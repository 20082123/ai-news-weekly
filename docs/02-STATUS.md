...
- **模仿期 4～8 篇**：套用「对标拆解·模仿期模板表 v1」（Vault `AI Signal/调研/`）写，
  管线降级为资料库，按需核验；主线周更继续；
- 模板表待补：封面样式与发布节奏（需用户截 3～5 张对标封面，或 OpenCLI 桥接恢复）；
- 真正的第一篇（受众匹配已过）：ai-agent-book，按模板公式重出标题后发布；
- 几天后回填 DeepSeek 第一条的平台数据；Golden Set 继续标注。
...
- **真正的第一篇（受众匹配已过）**：ai-agent-book 免费中文书——②直接落在
  ...
  终审后发布；
- **双栏目已定（DEC-024）**：主线「这周 AI 关你什么事」周更 + 副线
  「我搭的工具」图文先行；副线第一期规划在 Vault
  `AI Signal/栏目2-我搭的工具/第1期-规划.md`（工作台故事）；
