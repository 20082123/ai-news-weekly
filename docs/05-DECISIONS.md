...
1. 新增平台发布模板注册表（`outputs/platform_templates.py`）：每平台一份
   确定性「发稿说明书」（标题规则/正文结构/字数上限/标签规则/配图规格/
   来源位置/终审规则），小红书先行，其余平台按需追加；
2. 新增两个确定性输出命令：
   - `content prompt-pack`：组装护栏提示包（四问骨架 + 全部事实 +
     禁说清单 + 平台规则），交给豆包等外部模型出稿；
   - `content platform-draft`：渲染平台骨架稿——系统放好已验证零件
     （事实清单/禁说清单/配图清单/判定备忘），表达空位留白，
     由 agent 或外部模型填；
3. 分层结论：**系统不脱离 agent、也不全依赖 agent**——格式与组装是
   系统内确定性层（可测试），表达是外包生成层（可替换）；参考开源项目
   （文颜/PostSync 的平台元数据思路），但不引入其登录态自动发布
   （凭据永不落库铁律）。

### 原因

第一条发布的 outcome/lesson（表达一般、配图不会搭）证明：质量波动的
来源是每次手工发挥，解法是把「平台格式」做成可复用说明书——先完成后完善。

## DEC-022：加工与发布层独立成库（ai-signal-studio）
...
1. 新建独立仓库 `D:\xiangmu\ai-signal-studio`（暂未建远端）：负责
   封面卡片生成、截图标注、配图供给与多平台发布（S1-S4），本仓库只读；
2. 接口 = **目录即接口**：Studio 读 Vault `AI Signal/Drafts/`（骨架稿）与
   `Content/`（选题卡），唯一允许的 Vault 写入点是 `AI Signal/Images/`；
3. 骨架稿的节标题成为正式数据契约（Studio docs/05-DATA-CONTRACT.md）；
   契约变更需两边各留决策记录，上游先出、下游跟进；
4. 两条铁律延伸到 Studio：凭据永不落库（发布登录态只在用户浏览器）、
   图内文字只搬运不生产（AI 生图只做无文字背景）。

### 原因

本仓库的三铁律（凭据不落库/不接 LLM/零依赖）与图片渲染、浏览器发布
天然冲突；分开 = 边界干净、各自可替换。第一用户明确要求加工平台单独成项目。

### 后果与代价

- 配图相关能力从本仓库移出（DEC-021 的配图清单保留为规格说明书，
  执行在 Studio）；OPS 动作 8 配图一步改为「交给 Studio 生成」。

## DEC-023：小红书写法升级（调研驱动，与 Studio D-006 同步）
...
1. 依据 agent-reach 小红书同类内容实测调研（Vault
   `AI Signal/调研/小红书同类内容学习报告-2026-08-16.md`），升级
   xiaohongshu 平台模板：标题必须是立场句或数字+立场（禁纯新闻句）、
   开头第一人称、骨架稿新增「金句翻译」节、互动钩子必须是抛问题；
2. 数据契约随之变更（Studio D-006）：上游先出、下游跟进——本仓库的
   content platform-draft 已输出新节，Studio 解析层需容错旧格式。

### 原因

第一条内容复盘的延伸：内容没有「看的欲望」不是事实问题，是表达问题；
2,420 赞同类笔记证明立场、第一人称、金句、抛问题四个要素决定点击与评论。

### 后果与代价

- 骨架稿节标题 +1（金句翻译）；旧格式骨架稿仍可读（下游容错警告）；
- 选题优先级补充：免费资源/白嫖路径类 > 纯涨价通知类。

## DEC-024：账号双栏目结构——主线翻译 + 副线「我搭的工具」，图文先行
...
1. 账号两个栏目：
   - **主线「这周 AI 关你什么事」**：AI 变化的翻译 + 核验，面向学生，保持周更；
   - **副线「我搭的工具」**：第一用户用 AI 给自己造工具的过程记录
     （求职插件、工作台等 vibecoding 过程向），面向创作者/效率党，每周或
     双周 1 条，图文为主（小红书主图文）；
2. 副线内容公式：问题 → 搭法 → 踩坑（至少一个真实坑）→ 成品（真实数字）→ 钩子；
3. 防定位漂移：副线只讲自己真实在用的东西；主线模板与核验纪律不变；
   两个栏目共用同一信用（数字可查、不搞「最强/必装」安利）；
4. 副线选题池与第一期规划存 Vault `AI Signal/栏目2-我搭的工具/`。

### 原因

调研发现「用 DeepSeek Harness 搭工作台」类内容收藏率极高（287 赞/292 收藏），
且我们手里的不是演示是实盘；副线吃定位受众的「创作者」一半，素材零成本。

### 后果与代价

- 账号内容从单一资讯线变为双栏目；主线节奏不变；
- 「我搭的工具」类内容若数据好，可为未来产品化（Stage G 多用户）积累依据，
  但在反馈数据足够前不启动产品化。

## DEC-025：进入模仿期——先套爆款模板，系统降级为资料库
...
1. 账号运营进入**模仿期**（4～8 篇）：选题、标题、开头、封面严格套用
   同赛道爆款模板（拆解表在 Vault
   `AI Signal/调研/对标拆解-模仿期模板表-v1.md`）；
   唯一不模仿的是对标账号的毛病（数字无出处、夸大、标题党），禁说清单照用；
2. 模仿期系统**降级为资料库**：管线照常维护但不驱动发布节奏，需要核验
   事实时按需取用；「慢慢形成自己的内容创作」= 先像，再有意的偏差，
   偏差有数据了才叫风格；
3. 第一用户熟悉套路之后，再决定何时把管线重新提到台前（副线
   「我搭的工具」类内容天然适合展示管线，不着急）。
4. 账号自身的这次转向，正是 2B 教训的复用：工程（系统）没被推翻，
   被推翻的是「不验证就假设受众要什么」的方法。

### 后果与代价

- 选题优先级改为「模板数据驱动」而非「管线队列驱动」；主线周更继续；
- OpenCLI 浏览器桥接断开期间，对标账号的封面样式与发布节奏数据暂缺，
  恢复后补齐模板表 v2。
