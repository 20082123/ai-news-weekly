...
2. **每条新闻必须带链接**：新闻部分每一条 MUST 以 [查看详情](url) 结尾，url 从输入数据的"链接:"行原样复制。没有链接的新闻条目视为不合格。
3. **链接必须逐字复制**：每个 [文字](url) 中的 url 必须从输入数据的"链接:"行原样复制，一个字符都不能改。禁止使用"来源"作为链接文字。
4. **上手命令必须保守**：如果输入的README摘要里有具体安装命令，就用那个。如果没有，就写"详见仓库README"，绝不能编造 pip install/docker run 命令。
5. **星数必须如实**：输入写"未获取"就写"数据暂缺"，输入有数字就如实使用。绝对不能自己编一个数字。
6. **当数据缺失时的标准措辞**：
...
**本周AI重磅动态**
三小节，总字数400-600字。每一条新闻必须包含可点击的链接。
