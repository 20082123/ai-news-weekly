...
def render_prompt_pack(
    ...
    lines.append("- 开头必须第一人称（「我替你把官方 XX 算了一遍」），不写新闻导语。")
    lines.append("- 必须产出一句「金句翻译」：官方术语 → 大白话对比句（如「以前便宜到懒得算，现在不掏计算器都不敢点发送」）。")
    lines.append("- 结尾互动必须是一个问题，不是建议。")
...
def render_platform_skeleton(
    ...
    lines.append("____")
    lines.append("")
    lines.append("## 开头结论（1-2 行：第一人称「我替你把官方 XX 算/翻了一遍」+ 立场，待填）")
    lines.append("")
    lines.append("____")
    lines.append("")
    lines.append("## 金句翻译（一句大白话：官方术语 → 人话对比句，待填）")
    lines.append("")
    lines.append("____")
    lines.append("")
    ...
    lines.append("%s" % platform.source_rule)
    lines.append("")
    lines.append("## 互动钩子（必须抛一个问题引发评论，待填）")
    lines.append("")
    lines.append("____")
    lines.append("")
